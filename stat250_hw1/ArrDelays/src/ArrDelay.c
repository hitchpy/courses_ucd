...
double readDelays(const char *filename, Table *d, int fieldNum);


SEXP freq_table(SEXP filename) {
  const char *ff = CHAR(asChar(filename));
  ...
  readDelays(ff, &tt, FIELD_NUM);
  PROTECT(out = allocVector(INTSXP, MAX_NUM_VALUES));
  pout = INTEGER(out);
...
double
readDelays(const char *filename, Table *tt, int fieldNum)
{
    FILE *f;
